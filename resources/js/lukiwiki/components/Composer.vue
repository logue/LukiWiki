  ...
  <b-container fluid>
    ...
    >
      <template slot="name" slot-scope="row">
        <a :href="row.item.url" target="_blank">{{ row.item.name }}</a>
        <br />
        <small>{{ row.item.description }}</small>
      </template>
      <template slot="author" slot-scope="row">
        <span v-for="person in row.item.author" :key="person.id">
          <a
    ...
    <b-modal v-model="outputModal" title="Composer Output" no-close-on-backdrop>
      <div v-if="!outputBody">
        <b-spinner class="align-middle" />
      </div>
      <pre v-else>
