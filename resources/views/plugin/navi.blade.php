...
@if(isset($ret['prev']))
        <li><a class="btn page-link badge-pill" href="{{ url($ret['prev']) }}"><i class="fas fa-chevron-left"></i> {{ $ret['prev'] }}</a></li>
@else
...
@if(isset($ret['next']))
        <li><a class="btn page-link badge-pill" href="{{ url($ret['next']) }}">{{ $ret['next'] }} <i class="fas fa-chevron-right"></i></a></li>
@else
