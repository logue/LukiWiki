...
class WikiController extends Controller
{
    ...
    public function __invoke(Request $request, ?string $query = null): View
    {
        ...
        // ページを読み込み
        clock()->event(`Fetch $page data from db.`)->begin();
        $entry = $this->model->where('name', '=', $page)->first();
        if (! $entry) {
            // ページが見つからない
            abort(404, __('Page :page is not found.', ['page' => $page]));
        }
        clock()->event(`Fetch $page data from db.`)->end();

        // カウンタを更新
        clock()->event('Update counter.')->begin();
        // TODO:あまり速くない
        $counter = [
            'id' => Str::uuid(),
            'today' => 0,
        ...
        clock()->event('Update counter.')->end();
    ...
    {
        clock()->event(`Fetch $page backup data from db.`)->begin();
        ...
        clock()->event(`Fetch $page backup data from db.`)->end();
    ...
    public function search(Request $request): View
    {
        ...
        if (! empty($keywords)) {
            clock()->event(`Search $keywords...`)->begin();
            $entries = $this->model->search($keywords)->get()->toArray();
            clock()->event(`Search $keywords...`)->end();
        }
        // dd($entries);
