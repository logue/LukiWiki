...
class CreateAttachmentsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(self::TABLE_NAME, function (Blueprint $table) {
            $table->uuid('id')->primary()->comment('ファイル番号');
            $table->foreignUuid('page_id')->references('id')->on('pages')->onDelete('cascade')->comment('記事ID');
            $table->foreignUuid('user_id')->nullable()->references('id')->on('users')->comment('ユーザID');
